 ...
 * @since [ HG ] W 1.0
/*
Template Name: Dashboard - Editar Produto WooCommerce
*/
...
if (!$product || !$product->is_type('simple')) {
    // Redirect or show an error if product is not found or not simple
    wp_redirect(bloginfo('url') . '/todos-produtoswoo/'); // Redirect to product list
    exit;
}
...
$product_image_url = $product_image_id ? wp_get_attachment_image_url($product_image_id, 'thumbnail') : wc_placeholder_img_src();
    ...
    <div class="uk-container">
        <div class="flex">
            <div class="md:w-[100%]">
                    ...
                    <div class="px-6">
                        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                            <div class="space-y-1.5">
                                <h1 class="leading-none font-semibold text-xl">Editar Produto</h1>
                                <p class="text-muted-foreground text-sm">Editando produto simples #<?php echo esc_html($product_id); ?> - <?php echo esc_html($product_name); ?>.</p>
                            </div>
                ...
                <form id="editar-produto-form" class="space-y-6">
                    <input type="hidden" name="action" value="update_simple_product">
                    <input type="hidden" name="product_id" value="<?php echo esc_attr($product_id); ?>">
                    <?php wp_nonce_field('update_product_nonce', 'update_product_nonce_field'); ?>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="stock_status" id="stock_status" class="w-full border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 placeholder:text-gray-400 transition-all duration-200">
                                        <option value="instock" <?php selected($stock_status, 'instock'); ?>>Em estoque</option>
                                        <option value="outofstock" <?php selected($stock_status, 'outofstock'); ?>>Fora de estoque</option>
                                        <option value="onbackorder" <?php selected($stock_status, 'onbackorder'); ?>>Aceita encomenda</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="product_category" id="product_category" class="w-full border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 placeholder:text-gray-400 transition-all duration-200">
                                        ...
                                        $selected_category = !empty($current_categories) ? $current_categories[0] : '';
                                        
                                        foreach ($categories as $category) {
                                            $selected = selected($selected_category, $category['id'], false);
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="col-span-1 md:col-span-2">
                                    <label class="inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="manage_stock" name="manage_stock" value="yes" class="form-checkbox h-4 w-4 text-green-500 focus:ring-green-500 border border-gray-300 rounded" <?php checked($manage_stock, true); ?>>
                                ...
                                <div id="stock_quantity_field" class="<?php echo $manage_stock ? '' : 'hidden'; ?>">
                                    ...
                                    <input
                                        ...
                                        class="border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5 placeholder:text-gray-400 transition-all duration-200"
                                        placeholder="0"
                                        <?php echo $manage_stock ? 'required' : ''; ?>
                                    />
                                </div>
                            </div>
                        </div>
...
<div class="w-full">
    ...
    <?php if ($product_image_url && $product_image_url !== wc_placeholder_img_src()) : ?>
        <div class="mb-4 p-3 bg-gray-50 rounded-lg border">
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                    <img src="<?php echo esc_url($product_image_url); ?>" alt="Imagem Atual" class="w-16 h-16 object-cover rounded-lg ring-1 ring-gray-200">
                    <div>
                        <p class="text-sm font-medium text-gray-900">Imagem atual do produto</p>
                        <p class="text-xs text-gray-500">Clique abaixo para substituir ou marque para remover</p>
                    </div>
                </div>
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="remove_current_image" name="remove_current_image" value="yes" class="form-checkbox h-4 w-4 text-red-600 focus:ring-red-500 border border-gray-300 rounded">
                    <span class="ml-2 text-sm font-semibold text-red-700">Remover</span>
                </label>
            </div>
        </div>
    <?php endif; ?>
    ...
    <div id="upload-zone-produto" class="upload-zone border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-green-400 hover:bg-green-50 transition-all duration-300 cursor-pointer">
        ...
        <div id="upload-placeholder-produto" class="upload-placeholder">
            ...
            <p class="text-gray-600 font-medium">Arraste nova imagem ou clique para selecionar</p>
...
<script>
    // Classe ModernFileUpload (copiar completa do dashboard-novo-produtowoo.php)
...
$('#editar-produto-form').on('submit', function(e) {
    ...
    formData.append('action', 'update_simple_product');
    ...
    const nonceField = form.find('input[name="update_product_nonce_field"]');
    if (nonceField.length && nonceField.val()) {
        formData.append('update_product_nonce_field', nonceField.val());
    ...
    console.log('Nonce disponível no window.AmedisAjax:', window.AmedisAjax ? window.AmedisAjax.nonces.updateProduct : 'N/A');
    ...
    const requiredFields = ['action', 'product_id', 'update_product_nonce_field'];
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            if (res && res.success) {
                $('#success-message').text(res.data.message || 'Produto atualizado com sucesso!');
                UIkit.modal('#modal-success').show();

                // Atualizar preview da imagem atual se nova foi enviada
                if (res.data.new_image_url) {
                    const currentImg = $('.mb-4 img');
                    if (currentImg.length) {
                        currentImg.attr('src', res.data.new_image_url);
                    }
                    $('#remove_current_image').prop('checked', false);
                } else if (res.data.image_removed) {
                    $('.mb-4').remove();
                }
                ...
                const errorMsg = (res && res.data && res.data.message) ? res.data.message : 'Erro desconhecido ao salvar o produto.';
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            ...
            let errorMessage = 'Erro de conexão ao salvar o produto.';
