 ...
 * @since [ HG ] W 1.0
/*
Template Name: Editar Associado
*/
...
if (isset($_GET['editar_paciente'])) {
    $user_id = intval($_GET['editar_paciente']);
    ...
    $genero = get_user_meta($user_id, 'genero', true);
    $profissao = get_user_meta($user_id, 'profissao', true);

    $plano_escolhas = get_user_meta($user_id, 'plano_escolhas', true);
    $plano_saude = get_user_meta($user_id, 'plano_saude', true);
    ...
    $associado = get_user_meta($user_id, 'associado', true);
    $associado_ativado = get_user_meta($user_id, 'associado_ativado', true);
    ...
    $receita_laudo_escolhas = get_user_meta($user_id, 'receita_laudo_escolhas', true);
    $receita_paciente = get_user_meta($user_id, 'receita_paciente', true);
    ...
    $data_nascimento_respon = get_user_meta($user_id, 'data_nascimento_respon', true);
    $genero_responsavel = get_user_meta($user_id, 'genero_responsavel', true);
    $cpf_responsavel = get_user_meta($user_id, 'cpf_responsavel', true);
    $rg_responsavel = get_user_meta($user_id, 'rg_responsavel', true);
    $profissao_responsavel = get_user_meta($user_id, 'profissao_responsavel', true);

    $idade = get_user_meta($user_id, 'idade', true);
...
<div id="toast-success" class="flex items-center w-full max-w-full p-4 mb-4 text-green-600 bg-green-200 rounded-lg shadow" role="alert">
    ...
    <div class="ms-3 text-sm font-normal">Paciente editado com sucesso.</div>
...
<form action="" enctype="multipart/form-data" method="POST">
    <?php wp_nonce_field('editar_associado_action', 'editar_associado_nonce'); ?>
...
<div class="mb-5 card card-layout-frame">
    <div class="card-body">    
        <div class="flex justify-between space-x-2">
            <div>
                <h3 class="mb-1 text-lg uppercase font-semibold text-zinc-900">Associação:</h3>
                      
            </div>
            <div class="flex space-x-2">
                <div>
                            <label class="inline-flex items-center cursor-pointer">
                              <input type="checkbox" value="1" class="sr-only peer" id="associado" name="acf[field_66b23d4bf502a]" <?php if($associado) : ?>checked<?php endif; ?> disabled>
                              <div class="relative w-11 h-6 <?php if($associado) : ?>bg-green-200<?php else : ?>bg-gray-200<?php endif; ?> peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:bg-green-400 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:w-5 after:h-5 after:transition-all"></div>
                              <span class="ms-3 text-sm font-medium text-gray-900"><?php if($associado) : ?>Desativar<?php else : ?>Ativar<?php endif; ?> Associação</span>
                            </label>             
                </div>
                <div class="desabilitaAssoc">
                            <label class="inline-flex items-center cursor-pointer">
                              <input type="checkbox" value="1" class="sr-only peer autoChecked" id="associado_ativado" name="acf[field_66b252b04990d]" <?php if($associado_ativado) : ?>checked<?php endif; ?>>
                              <div class="relative w-11 h-6 <?php if($associado_ativado) : ?>bg-green-200<?php else : ?>bg-gray-200<?php endif; ?> peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:bg-green-400 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:w-5 after:h-5 after:transition-all"></div>
...
<span class="block text-sm mt-5 hidden">Selecione o tipo desejado abaixo *</span>   
<ul class="grid w-full gap-6 md:grid-cols-4 mt-5 mb-5 hidden" uk-height-match="target: > li > label">
    <li class="">
        <input type="radio" id="assoc_paciente" name="acf[field_66b40ca7a5636]" value="assoc_paciente" class="hidden  peer" <?php checked($tipo_associacao, 'assoc_paciente'); ?>>
    ...
    <li class="">
        <input type="radio" id="assoc_respon" name="acf[field_66b40ca7a5636]" value="assoc_respon" class="hidden  peer" <?php checked($tipo_associacao, 'assoc_respon'); ?>>
        <label for="assoc_respon" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer  peer-checked:border-green-600 peer-checked:bg-green-50 hover:text-gray-600 peer-checked:text-gray-600 hover:bg-gray-50">                           
            <div class="block flex space-x-2">
                ...
                <div>
                    ...
                    <div class="w-full text-xs">Associação para responsável por menor de 18 anos.</div>
                </div>
            </div>
        </label>
    </li>
    <li class="">
        <input type="radio" id="assoc_tutor" name="acf[field_66b40ca7a5636]" value="assoc_tutor" class="hidden  peer" <?php checked($tipo_associacao, 'assoc_tutor'); ?>>
    ...
    <li class="">
        <input type="radio" id="assoc_colab" name="acf[field_66b40ca7a5636]" value="assoc_colab" class="hidden  peer" <?php checked($tipo_associacao, 'assoc_colab'); ?>>
        <label for="assoc_colab" class="inline-flex items-center justify-between w-full p-5 text-gray-500 bg-white border-2 border-gray-200 rounded-lg cursor-pointer  peer-checked:border-green-600 peer-checked:bg-green-50 hover:text-gray-600 peer-checked:text-gray-600 hover:bg-gray-50">                           
          ...
          <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">
              ...
              <div class="sm:col-span-2">
                  ...
                  <input type="text" name="acf[field_666c8794b6c48]" id="nome_completo" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Nome completo paciente" value="<?php echo esc_attr($nome_completo); ?>" required="">
              </div>
              ...
              <div class="w-full mostra-tutor" style="display:none;">
                  ...
                  <input type="text" name="acf[field_66dc4d9b7f708]" id="idade" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Idade do paciente" value="<?php echo esc_attr($idade); ?>" required="">
              </div>
                ...
                <div class="w-full esconde-responsavel">
                    ...
                    <input type="email" name="email_paciente" id="email_paciente" class="email-temp bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="E-mail do paciente"  value="<?php echo esc_attr($email); ?>" readonly/>
                </div>
              ...
              <div class="w-full esconde-tutor">
                  ...
                  <input type="text" name="acf[field_66db09c620956]" id="data_nascimento" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Data nascimento do Paciente" value="<?php echo esc_attr($data_nascimento); ?>" required="">
              </div> 
        ...
        <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">
              ...
              <div class="w-full esconde-tutor">
                  ...
                  <input type="text" name="acf[field_666c87a7b6c49]" id="cpf" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="CPF do Paciente" value="<?php echo esc_attr($cpf); ?>" required="">
              </div>   

              <div class="w-full esconde-tutor">
                  <label for="rg" class="block mb-2 text-sm font-medium text-gray-900">RG *</label>
                  <input type="text" name="acf[field_66db09b0b6165]" id="rg" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="RG do Paciente" value="<?php echo esc_attr($rg); ?>" required="">
              </div>
                ...
                <div class="w-full esconde-tutor">
                    ...
                    <select id="genero" name="acf[field_66db09e620957]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                        <option value="" disabled <?php selected($genero, ''); ?>>Selecione uma opção</option>
                        <option value="masculino" <?php selected($genero, 'masculino'); ?>>Masculino</option>
                        <option value="feminino" <?php selected($genero, 'feminino'); ?>>Feminino</option>
                        <option value="naobinario" <?php selected($genero, 'naobinario'); ?>>Não-Binário</option>
                        <option value="naoinformar" <?php selected($genero, 'naoinformar'); ?>>Não desejo informar</option>
                    </select>
                </div>
              ...
              <div class="w-full esconde-responsavel">
                  ...
                  <input type="text" name="acf[field_66db0a3f8d587]" id="profissao" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Profissão do Paciente" value="<?php echo esc_attr($profissao); ?>">
              </div>
            ...
            <div class="w-full">
                ...
                <select id="lano_escolhas" name="acf[field_66db0a604696d]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                    <option value="" disabled <?php selected($plano_escolhas, ''); ?>>Selecione uma opção</option>
                    <option value="nao" <?php selected($plano_escolhas, 'nao'); ?>>Não</option>
                    <option value="sim" <?php selected($plano_escolhas, 'sim'); ?>>Sim</option>
                </select>
            </div>
            ...
            <div class="w-full mostraPlano" style="display:none;">
                ...
                <input type="text" name="acf[field_66db0a9e2e313]" id="plano_saude" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Digite Aqui" value="<?php echo esc_attr($plano_saude); ?>">
            </div>
        ...
        <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">
              <div class="sm:col-span-2">
                  ...
                  <input type="text" name="acf[field_666c87b0b6c4a]" id="endereco" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Endereço do Paciente" value="<?php echo esc_attr($endereco); ?>" required>
              </div>

              <div class="w-full">
                  <label for="numero" class="block mb-2 text-sm font-medium text-gray-900">Número *</label>
                  <input type="text" name="acf[field_66db0ac742700]" id="numero" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Número do Paciente" value="<?php echo esc_attr($numero); ?>" required>
              </div> 

              <div class="w-full">
                  <label for="bairro" class="block mb-2 text-sm font-medium text-gray-900">Bairro *</label>
                  <input type="text" name="acf[field_666c87b6b6c4b]" id="bairro" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Bairro do Paciente" value="<?php echo esc_attr($bairro); ?>" required>
              </div>


              <div class="sm:col-span-3">
                  <label for="complemento" class="block mb-2 text-sm font-medium text-gray-900">Completemento / Referência</label>
                  <input type="text" name="acf[field_66db0adb42701]" id="complemento" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Complemento Endereço do Paciente" value="<?php echo esc_attr($complemento); ?>" >
              </div>


              <div class="w-full">
                  <label for="cep" class="block mb-2 text-sm font-medium text-gray-900">CEP *</label>
                  <input type="text" name="acf[field_66db0af08561d]" id="cep" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="CEP do Paciente" value="<?php echo esc_attr($cep); ?>" required>
              </div>
          </div>
          <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">
              ...
              <div class="w-full">
                  ...
                  <input type="text" name="acf[field_666c87bcb6c4c]" id="cidade" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Cidade do Paciente" value="<?php echo esc_attr($cidade); ?>" required>
              </div> 
    ...
    <select id="select-uf" name="select-uf" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
        ...
        <option value="to">Tocantins</option>
                  ...
                  <input type="text" name="acf[field_66db0b0e8561e]" id="estado" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Estado do Paciente" value="<?php echo esc_attr($estado); ?>" hidden required>
              </div>                 
              <div class="w-full esconde-responsavel">
                  <label for="telefone_paciente" class="block mb-2 text-sm font-medium text-gray-900">Telefone *</label>
                  <input type="text" name="telefone_paciente" id="telefone_paciente" class="tel-temp bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Celular do Paciente" value="<?php echo esc_attr($telefone); ?>" required="">
              </div>  
        ...
        <div class="">
              ...
              <div class="col-start-1 sm:col-span-4">
                    ...
                    <textarea id="diagnostico" name="acf[field_66db3419a6abf]" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-green-500 focus:border-green-500" placeholder="Digite aqui..." required><?php echo esc_attr($diagnostico); ?></textarea>
              </div>
                ...
                <div class="w-full">
                    ...
                    <select id="usa_medicacao" name="acf[field_66db346780264]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                        <option value="" disabled <?php selected($usa_medicacao, ''); ?>>Selecione uma opção</option>
                        <option value="nao" <?php selected($usa_medicacao, 'nao'); ?>>Não</option>
                        <option value="sim" <?php selected($usa_medicacao, 'sim'); ?>>Sim</option>
                    </select>
                </div>
                ...
                <div class="w-full">
                    ...
                    <select id="fez_uso_canabis_escolha" name="acf[field_66db34a5824c8]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                        <option value="" disabled <?php selected($fez_uso_canabis_escolha, ''); ?>>Selecione uma opção</option>
                        <option value="nao" <?php selected($fez_uso_canabis_escolha, 'nao'); ?>>Não</option>
                        <option value="sim" <?php selected($fez_uso_canabis_escolha, 'sim'); ?>>Sim</option>
                    </select>
                </div>
                ...
                <div class="col-start-1 sm:col-span-2">
                    ...
                    <select id="medico_canabis_escolhas" name="acf[field_66db34cd0310a]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                        <option value="" disabled <?php selected($medico_canabis_escolhas, ''); ?>>Selecione uma opção</option>
                        <option value="nao" <?php selected($medico_canabis_escolhas, 'nao'); ?>>Não</option>
                        <option value="sim" <?php selected($medico_canabis_escolhas, 'sim'); ?>>Sim</option>
                    </select>
                </div>
            </div>
            <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">
              ...
              <div class="mostraMedicacao w-full" style="display:none;">
                  ...
                  <input type="text" name="acf[field_66db348c6b29d]" id="qual_medicacao" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Digite aqui" value="<?php echo esc_attr($qual_medicacao); ?>" required="">
              </div> 
              ...
              <div class="mostraMPrescritor w-full" style="display:none;">
                  ...
                  <input type="text" name="acf[field_66db34fbb2202]" id="nome_profissional" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Digite aqui" value="<?php echo esc_attr($nome_profissional); ?>" required="">
              </div> 

              <div class="mostraMPrescritor w-full" style="display:none;">
                  <label for="crm_profi" class="block mb-2 text-sm font-medium text-gray-900">CRM / UF do profissional  *</label>
                  <input type="text" name="acf[field_66db3509b2203]" id="crm_profi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Digite aqui" value="<?php echo esc_attr($crm_profi); ?>" required="">
              </div>               
            ...
            <div class="card card-layout-frame my-8">
              ...
              <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">
                  <div class="sm:col-span-2">
                      ...
                      <input type="text" name="acf[field_66dc418137429]" id="nome_completo_respon" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Nome completo responsável" value="<?php echo esc_attr($nome_completo_respon); ?>" required="">
                  </div> 
                    ...
                    <div class="w-full mostra-responsavel">
                        ...
                        <input type="email" name="email_responsavel" id="email_responsavel" class="email-temp bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="E-mail do responsável" value="<?php echo esc_attr($email); ?>" readonly/>
                    </div>

                  <div class="w-full">
                      <label for="data_nascimento_respon" class="block mb-2 text-sm font-medium text-gray-900">Data de Nascimento *</label>
                      <input type="text" name="acf[field_66dc41a93742a]" id="data_nascimento_respon" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Data de Nascimento do responsável" value="<?php echo esc_attr($data_nascimento_respon); ?>" required="">
                  </div>
                  ...
                  <div class="w-full">
                      ...
                      <select id="genero_responsavel" name="acf[field_66dc41b93742b]" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" required>
                          <option value="" disabled <?php selected($genero_responsavel, ''); ?>>Selecione uma opção</option>
                          <option value="masculino" <?php selected($genero_responsavel, 'masculino'); ?>>Masculino</option>
                          <option value="feminio" <?php selected($genero_responsavel, 'feminio'); ?>>Feminino</option>
                          <option value="naobinario" <?php selected($genero_responsavel, 'naobinario'); ?>>Não-Binário</option>
                          <option value="naoinformar" <?php selected($genero_responsavel, 'naoinformar'); ?>>Não desejo informar</option>
                      </select>
                  </div>

                  <div class="w-full">
                      <label for="cpf_responsavel" class="block mb-2 text-sm font-medium text-gray-900">CPF *</label>
                      <input type="text" name="acf[field_66dc41f6e8aea]" id="cpf_responsavel" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="CPF do responsável" value="<?php echo esc_attr($cpf_responsavel); ?>" required="">
                  </div>

                  <div class="w-full">
                      <label for="rg_responsavel" class="block mb-2 text-sm font-medium text-gray-900">RG *</label>
                      <input type="text" name="acf[field_66dc4201e8aeb]" id="rg_responsavel" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="RG do responsável" value="<?php echo esc_attr($rg_responsavel); ?>" required="">
                  </div>    

                  <div class="w-full">
                      <label for="profissao_responsavel" class="block mb-2 text-sm font-medium text-gray-900">Profissão *</label>
                      <input type="text" name="acf[field_66dc4211e8aec]" id="profissao_responsavel" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Profissão do responsável" value="<?php echo esc_attr($profissao_responsavel); ?>" required="">
                  </div>  
                  <div class="w-full">
                      <label for="telefone_respon" class="block mb-2 text-sm font-medium text-gray-900">Telefone *</label>
                      <input type="text" name="telefone_respon" id="telefone_respon" class="tel-temp bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full p-2.5" placeholder="Celular do Responsável" value="<?php echo esc_attr($telefone); ?>" required="">
                  </div> 
              </div>
          </div>
          
<?php 
$comprova_end_paciente_url = wp_get_attachment_url($comprova_end_paciente); 
$comprova_end_paciente_name = basename(get_attached_file($comprova_end_paciente)); 

$comprova_rg_paciente_url = wp_get_attachment_url($comprova_rg_paciente); 
$comprova_rg_paciente_name = basename(get_attached_file($comprova_rg_paciente)); 

$receita_paciente_url = wp_get_attachment_url($receita_paciente); 
$receita_paciente_name = basename(get_attached_file($receita_paciente));

$laudo_paciente_url = wp_get_attachment_url($laudo_paciente); 
$laudo_paciente_name = basename(get_attached_file($laudo_paciente));

$termo_associativo_url = wp_get_attachment_url($termo_associativo); 
$termo_associativo_name = basename(get_attached_file($termo_associativo));
                ...
                <div class="grid gap-4 sm:grid-cols-4 sm:gap-4 mb-4">    
                    ...
                    <div class="w-full file-upload-container">
                        <div class="w-full">
                            ...
                            <?php if ($comprova_rg_paciente_url) : ?>
                                <a href="<?php echo $comprova_rg_paciente_url; ?>" target="_blank" class="flex items-center justify-between w-full block rounded-lg bg-blue-50 p-3 mb-2">
                    ...
                    <div class="w-full file-upload-container">
                        <div class="w-full">
                            ...
                            <?php if ($comprova_end_paciente_url) : ?>
                                <a href="<?php echo $comprova_end_paciente_url; ?>" target="_blank" class="flex items-center justify-between w-full block rounded-lg bg-blue-50 p-3 mb-2">
                                    <span>Visualizar Documento Atual</span>
                                    <span>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 8.25H7.5a2.25 2.25 0 0 0-2.25 2.25v9a2.25 2.25 0 0 0 2.25 2.25h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25H15M9 12l3 3m0 0 3-3m-3 3V2.25" />
                    ...
                    <div class="w-full file-upload-container">
                        <div class="w-full">
                            ...
                            <?php if ($laudo_paciente_url) : ?>
                                <a href="<?php echo $laudo_paciente_url; ?>" target="_blank" class="flex items-center justify-between w-full block rounded-lg bg-blue-50 p-3 mb-2">
                                    <span>Visualizar Documento Atual</span>
                                    <span>
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M9 8.25H7.5a2.25 2.25 0 0 0-2.25 2.25v9a2.25 2.25 0 0 0 2.25 2.25h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25H15M9 12l3 3m0 0 3-3m-3 3V2.25" />
...
<div class="w-full file-upload-container">
    <div class="w-full">
        <label class="block mb-2 text-sm font-medium text-gray-900">Termo Associativo</label>
        ...
        <?php if ($termo_associativo_url) : ?>
            <a href="<?php echo $termo_associativo_url; ?>" target="_blank" class="flex items-center justify-between w-full block rounded-lg bg-blue-50 p-3 mb-2">
                <span>Visualizar Documento Atual</span>
                <span>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 8.25H7.5a2.25 2.25 0 0 0-2.25 2.25v9a2.25 2.25 0 0 0 2.25 2.25h9a2.25 2.25 0 0 0 2.25-2.25v-9a2.25 2.25 0 0 0-2.25-2.25H15M9 12l3 3m0 0 3-3m-3 3V2.25" />
        ...
        <?php endif; ?>
                ...
                <div class="card card-layout-frame my-8">
                  ...
                  <div class="col-start-1 sm:col-span-4">
                        <textarea id="observacoes" name="acf[field_666c87e6b6c4d]" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-green-500 focus:border-green-500" placeholder="Digite aqui..."><?php echo esc_attr($observacoes); ?></textarea>
                  </div>
            ...
            <div id="stickyFooter_" class="w-full mt-5 px-6 flex items-center border border-gray-300 justify-between py-4 bg-white rounded-lg">
                ...
                <div class="md:flex items-center">
                    ...
                    <button class="btn btn-sm bg-green-500 text-white hover:opacity-50" type="submit" name="submit_editar_associado">
...
jQuery(document).ready(function($) {
    ...
    var estadoSalvo = $('#estado').val();
    $('#select-uf').val(estadoSalvo.toLowerCase()); // Tentando com minúsculas
    ...
    function atualizarCampoOculto() {
        ...
        var telResponsavel = $('#telefone_respon').val().trim();
